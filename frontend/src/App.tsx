...
import { Download, HelpCircle, LogOut, RefreshCw, UserRound } from "lucide-react";
...
const catalog = [
  { key: "horses" as const, label: "Адуу", iconSrc: "/module-icons/horse.png", alt: "Адууны модуль" },
  { key: "cattle" as const, label: "Үхэр", iconSrc: "/module-icons/cattle.png", alt: "Үхрийн модуль" },
  { key: "small_livestock" as const, label: "Хонь", iconSrc: "/module-icons/sheep.png", alt: "Хонины модуль" },
  { key: "analytics" as const, label: "Анализ", iconSrc: "/module-icons/analytics.png", alt: "Аналитикийн модуль" },
];
...
export default function App() {
  ...
  const sync = <SyncPanel userId={user.id} online={pwa.online} />;
  if (view === "horses") return <>{sync}<HorsePage onBack={() => setView("home")} /></>;
  if (view === "cattle") return <>{sync}<CattlePage onBack={() => setView("home")} /></>;
  if (view === "small_livestock") return <>{sync}<SheepPage onBack={() => setView("home")} /></>;
  if (view === "analytics") return <>{sync}<AnalyticsPage onBack={() => setView("home")} openTool={setView} /></>;
  if (["finance", "herders", "audit", "snapshots", "reports"].includes(view)) return <>{sync}<OwnerToolPage tool={view} onBack={() => setView("analytics")} /></>;
  ...
  return <main className="app-shell">
    ...
    <section className="home-grid">{allowed.map(({ key, label, iconSrc, alt }) => <button className="home-card" key={key} onClick={() => setView(key)}><img className="module-icon" src={iconSrc} alt={alt} /><span>{label}</span>{key !== "analytics" && <strong>{counts[key] ?? "—"}</strong>}</button>)}</section>
