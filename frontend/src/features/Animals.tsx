...
import { ArrowLeft, Camera, History, Pencil, Plus, RotateCcw } from "lucide-react";
import { api, NetworkError } from "../api/client";
import { useAuth } from "../auth/AuthContext";
import { enqueueMutation } from "../offline/queue";
import type { Cattle, Horse, HorseGroup, Stats } from "../types";
...
export function HorsePage({ onBack }: { onBack(): void }) {
  ...
  return <main className="page-shell">
    ...
    {selected && !editing && <Modal title={selected.display_label} onClose={() => setSelected(null)} wide>
      ...
      <div className="image-strip">{selected.images.map((image) => <img key={image.id} src={image.url} alt={image.original_filename} />)}</div>
      ...
      <div className="button-row">{selected.archived_at ? <button className="primary" onClick={restore}><RotateCcw /> Сэргээх</button> : <><button className="secondary" onClick={() => beginEdit(selected)}><Pencil /> Засах</button><button className="secondary" onClick={() => setTransferOpen(true)}>Бүлэг солих</button><button className="danger" onClick={() => setArchiveOpen(true)}>Архивлах</button></>}<button className="ghost" onClick={() => void showHistory()}><History /> Түүх</button></div>
...
export function CattlePage({ onBack }: { onBack(): void }) {
  ...
  async function upload(files: FileList | null) { if (!selected || !files?.length || !navigator.onLine) { setError("Зураг онлайн үед илгээгдэнэ."); return; } const data = new FormData(); Array.from(files).forEach((file) => data.append("files", file)); setSelected(await api(`/api/v1/cattle/${selected.id}/images`, { method: "POST", body: data })); await load(); }
  return <main className="page-shell">
